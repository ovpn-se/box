    ...
    if(!empty($config['openvpn']['openvpn-client'])) {
        foreach($config['openvpn']['openvpn-client'] as $key => $client) {
            ...
            if($client['vpnid'] == $vpnID) {
                ...
                $config['openvpn']['openvpn-client'][$key]['custom_options'] = 'remote ' . $ip . ' ' . $ports[1] . ';remote-random;remote-cert-tls server;reneg-sec 432000;mute-replay-warnings;replay-window 256;tls-auth /var/etc/openvpn/ovpn-tls.key 1;log /tmp/openvpn.log';
